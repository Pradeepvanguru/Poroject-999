...
function Table() {
  ...
  const fetchUserDetails = async (acceptedBy) => {
    try {
      const response = await axios.get(`https://project-l5.onrender.com/api/registers/${acceptedBy}`);
  ...
  const fetchAcceptedRequests = async () => {
    setLoading(true);
    try {
      const response = await axios.get('https://project-l5.onrender.com/api/leave');
