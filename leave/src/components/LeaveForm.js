...
function LeaveForm() {
  ...
  const onSubmit = (data, event) => {
    ...
    axios.post('https://project-l5.onrender.com/api/leave', data)
      .then((response) => {
        reset();
        navigate('/');
      })
  ...
  const fetchFacultyData = async ({ email, branch, sem, year }) => {
    try {
      const response = await axios.post('https://project-l5.onrender.com/api/getFacultyData', { email, branch, sem, year });
      if (response.data) {
